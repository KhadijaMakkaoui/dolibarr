...
/**
 * \file    test/phpunit/StagiaireFunctionalTest.php
 * \ingroup stagiaire
 ...
 *
 * @package Teststagiaire
 */
class StagiaireFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modStagiaire')->click();
		} else {
			// Disable the module
			$this->byHref('modStagiaire')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/stagiaire/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('stagiaire/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/stagiaire/admin/about.php');
		$this->authenticate();
		return $this->assertContains('stagiaire/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('stagiairewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modStagiaire_StagiaireTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modStagiaire_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
